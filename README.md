...
- `project_name`: the default docker compose project name (default: nautobot_ssot_db_stada)
...
Each command can be executed with `invoke <command>`. Environment variables `INVOKE_NAUTOBOT_SSOT_DB_STADA_PYTHON_VER` and `INVOKE_NAUTOBOT_SSOT_DB_STADA_NAUTOBOT_VER` may be specified to override the default versions. Each command also has its own help `invoke <command> --help`
